...
async function fetchOwnerFullNamesBatch(ownerCodes) {
    ...
    try {
        const res = await fetch("http://10.220.130.119:9090/api/SearchFA/get-fullname-batch", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
...
async function loadData(startDate, endDate) {
    try {
        const [repair, confirm, viReConfirm, checkListConfirm] = await Promise.all([
            ...
            fetchData("http://10.220.130.119:9090/api/SearchFA/get-vi-re-confirm-data", { startDate, endDate }),
            fetchData("http://10.220.130.119:9090/api/SearchFA/get-check-list-confirm-data", { startDate, endDate })
        ]);
