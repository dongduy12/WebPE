// Bonepile After dashboard script aligned with Index.cshtml layout
// and Bonepile2 web API responses.
document.addEventListener('DOMContentLoaded', async function () {
    ...
    const apiCheckInOutBase = 'http://10.220.130.119:9090/api/CheckInOut';
    ...
    function updateKpiGridLayout() {
        ...
        const maxColumns = 3;
        const availableWidth = grid.clientWidth || grid.offsetWidth;
        if (!availableWidth) {
            return;
        }
        const calculatedColumns = Math.max(1, Math.min(maxColumns, Math.floor(availableWidth / minCardWidth)));
        grid.style.gridTemplateColumns = `repeat(${calculatedColumns}, minmax(${minCardWidth}px, 1fr))`;
        grid.style.gridTemplateRows = 'auto';
        grid.style.gridAutoRows = 'minmax(70px, auto)';
    ...
    function attachTooltipEvents() {
        $(document)
            .on('mouseover', '.tooltip-trigger', function (e) {
                const $this = $(this);
